...
**Now:** DONE. All 13 DoD boxes ticked. The 12-file change is on `metalllinux/team-chaotix` main
(merged sha `2c3b171`, PR #1) and verified on GitHub via the API: all 10 agent files, `AGENTS.md`, and
`README.md` show IQ4_XS / `evo-x2-qwen3.8-iq4xs` / 8093, zero stale refs. Commits unsigned (GPG not
configured on this host, matching current practice). Follow-ups recorded in `## Release`: GPG
provisioning, runner registration (repo has zero registered runners, so the PR CI runs were cancelled),
and the HF-revision pin (Omega low). Dispatching `Espio` to prune, then marking the task Done.
...
- [x] All 10 files in `.opencode/agents/` have exactly `model: evo-x2-qwen3.8-iq4xs/Qwen3.8-27B-UD-IQ4_XS`
  on frontmatter line 4. Verified by `grep -n "^model:" .opencode/agents/*.md` returning 10 identical lines.
- [x] `AGENTS.md` Model line (line 17) reads: all agents use `Qwen3.8-27B-UD-IQ4_XS`, endpoint
  `evo-x2-qwen3.8-iq4xs`, port 8093, `--parallel 1`.
- [x] `README.md`: the "All agents use X" statement (was line 393) and the "agents run on X" sentence
  (was line 400) read IQ4_XS / `evo-x2-qwen3.8-iq4xs` / 8093 / `--parallel 1`.
- [x] `README.md` `## EVO-X2 model host setup` section documents the IQ4_XS host: unit
...
- [x] `README.md` firewall subsection documents the firewalld rule (or verified current state) for port
  8093 on the EVO-X2.
- [x] No stale Q4_K_XL references in team config: `git ls-files | grep -v '^planning/' | xargs grep -l
  "Q4_K_XL\|evo-x2-qwen3.8-q4\|:8092\|port 8092"` returns nothing.
- [x] The IQ4_XS endpoint serves the model: `curl http://192.168.1.106:8093/v1/models` returns HTTP 200
  and lists `Qwen3.8-27B-UD-IQ4_XS`. (`Big` re-confirms at test time.)
- [x] All 10 agent frontmatter blocks parse as valid YAML with the expected model (frontmatter parse
  check, as in TASK-0011).
- [x] `Shadow`: no unresolved blockers or should-fix findings in `## Review` (should-fix resolved in
  `26b36f0`; the remaining GPG nit is a release-phase item, not a should-fix).
- [x] `Omega`: no unresolved findings above `low` in `## Security` (1 low, non-blocking, pre-existing;
  fix tracked as a follow-up).
- [x] `Big`: all harness checks PASS, with no silently dropped checks (5/5 requested + supplementary;
  no code matrix exists for a config change, stated not implied).
- [x] `Vector`: README updated as affected and house style (AGENTS.md §10) respected (no em/en dashes,
  no colons introducing explanations, no banned words).
- [x] `Knuckles`: commit pushed to `metalllinux/team-chaotix` main; the pushed agent files, `AGENTS.md`,
...
No open actions. Task DONE (2026-09-09); the doc was pruned by `Espio` the same day (see
`## Archive`). Follow-ups live in `## Release`.
...
**Assumptions** (plan in pencil; each is marked where it bites) — A1-A7, all resolved during
execution: A1 confirmed by the canary (the post-commit Shadow/Omega/Big dispatches ran on IQ4_XS,
`## Test Results`); A2 verified (Q4 unit `active`, `## Implementation` state record); A3 verified
(`sudo firewall-cmd` over ssh worked, `## Implementation`); A4 verified (live unit matches the
`## Status` `ExecStart` line for line); A5 verified (mmproj SHA matches the live `sha256sum`);
A6 and A7 followed per `## Release` (in-account autonomous merge, bookkeeping commit, `TASK-0022:`
message style). Original text in `## Archive`.

**Work breakdown** — 16 items, one agent finishes one item in one turn (the single inference slot
serializes everything): Tails 1-10 (branch; the ten model lines; the `AGENTS.md:17` line; README
`## Model` + EVO-X2 opener; Model files; systemd unit; opencode client JSON; Firewall; Verification
after setup; residual sweep + commit), Shadow 11, Omega 12, Big 13, Tails fix pass 14 (only if
findings), Vector 15, Knuckles 16. Items 2-9 are independent in content (different files, or
disjoint README regions) and freely orderable; only item 10 depends on all of them, and the review
trio is a fixed serial tail (AGENTS.md §3). All 16 items were executed; the original table, the
critical path, and the turn estimates (sum ≈ 14.0; plan for 14-16 turns) are in `## Archive`.

**Risks** — 7 risks assessed at planning time; outcomes: risk 4 (stale references the DoD grep
misses) materialized as documented — the README carried `8092`, the old unit name, the old gguf
SHA `3f227079…`, and bare `Q4` strings that match none of the four DoD patterns, and they were
caught and cleared by the item-10 extended sweep (`8092|27b-q4|3f227079|\bQ4\b`), a sweep pattern
future model switches should keep using. Risk 2 (8092 no longer listening) did not materialize:
the Q4 unit was `active` and 8092 listening (`## Implementation` state record), so the rollback
target is alive. Risks 1, 3, 5, 6, 7 did not materialize (no quality regression in the canary
window, `## Test Results`; the firewalld-for-8093 verification succeeded via ssh + sudo; no YAML
typo; post-commit dispatches picked up the new frontmatter; no planning file swept into the
commits). Original text in `## Archive`.

**Validation** — 6 criteria: DoD 1-3 machine checks, the DoD grep + extended sweep, the
TASK-0011 YAML parse, the live endpoint, the post-commit canary (empty result or 32k truncation
in that window would have been the rollback trigger), and the pushed files on GitHub. All met,
see `## Test Results` and `## Release`. Original text in `## Archive`.
...
**Checks run** (full session detail in `## Archive`):
- `grep -n "^model:" .opencode/agents/*.md` → 10 identical lines
  `model: evo-x2-qwen3.8-iq4xs/Qwen3.8-27B-UD-IQ4_XS`; `git diff --numstat` → 1 line/file for the
  9 non-vector agents, 2 for vector.md (see Alternatives).
...
- Extended sweep `grep -rEn "8092|27b-q4|3f227079|\bQ4\b" README.md AGENTS.md .opencode/agents/` → no
  matches (`rg` is not installed on this host; GNU `grep -rEn` used as the equivalent, `\b` included).
- Frontmatter YAML parse (PyYAML, the TASK-0011 check): 10/10 parse; `model` field asserted equal in each.
- README opencode client block parses as JSON; key `evo-x2-qwen3.8-iq4xs`, `baseURL`
  `http://192.168.1.106:8093/v1`.
- Live host, `ssh howard@192.168.1.106` (read 2026-09-09): `sudo -n firewall-cmd --zone=public --list-all`
  → `public` zone, `eno1`, ports 8080-8088, 8090, 8092, 8093 (all /tcp), services `cockpit dhcpv6-client ssh`.
  **8093/tcp is open** — the firewalld-for-8093 item (plan item 8) is resolved with the exact verified rule.
- Same session: `systemctl --user is-active` → iq4xs unit `active` and Q4 unit `active`; `ss -tln` →
  `0.0.0.0:8092` and `0.0.0.0:8093` both listening; `cat ~/.config/systemd/user/llama-server-qwen3.8-27b-iq4xs.service`
  matches the `## Status` verified `ExecStart` line for line (A4 cross-check passes); `sha256sum` in
  `/mnt/data/models/qwen3.8-27b-iq4xs/` → gguf `40fac405…e6199`, mmproj `cbb841a9…e43e`, both matching
  `## Status` (A5: mmproj byte-identical across quants, confirmed).
- Commit hygiene: `git diff --cached --stat` → exactly 12 files; `git show --name-only HEAD | grep -c planning`
  → `0` (no planning file swept in). Commit `fe83073` on `feature/TASK-0022-iq4xs-model`, unsigned (see GPG item above).
...
sentence cited the Q4-era read-back (2026-08-29) and the TASK-0010 live-process record, which cannot
contain the current iq4xs `ExecStart`. Repointed per Shadow's suggested direction: read-back now dated
2026-09-09 and citing the TASK-0022 record, `ExecStart` match citing the live unit verified in that
record, remaining lines keeping the `add-ai-model` template provenance. Commit `26b36f0` on
`feature/TASK-0022-iq4xs-model` (staged by explicit path; `README.md` only, 3 insertions / 3 deletions).
DoD grep and item-10 extended sweep re-run clean. Re-confirmation in `## Review`; full text in `## Archive`.
...
**Verified clean, no findings.** Independently re-verified against the commit: the 10 identical
`^model:` lines (all on line 4; the commit lists exactly the 10 agent files plus `AGENTS.md` and
`README.md`, 12 files, zero planning files; per-file diff one changed line, two for `vector.md`),
`AGENTS.md:17` reading the DoD wording verbatim, the residue sweep (DoD patterns and the item-10
extended sweep both zero hits in `README.md`, `AGENTS.md`, `.opencode/agents/`; repo-wide every hit
sits in `planning/`, historical task records correctly retained; the old unit name appears in no
live team config), the A5 mmproj SHA (`cbb841a9…e43e` matches `## Status` and the live `sha256sum`,
so the "byte-identical across quantizations" claim rests on a live hash), the coherent
one-active-model paragraph (the "only listener" claim is gone; the transitional both-ports-up state
is recorded in `## Implementation` per chosen Option B, and the README stays a reference-setup doc),
the firewall-for-8093 subsection (every stated fact matches the 2026-09-09 live read; the 8092/tcp
omission is the documented Option B, not a completeness claim), and the unit block, opencode client
JSON block, and verification list (all matching the `## Status` verified facts). Per-bullet detail
in `## Archive`.
...
acceptance, documented in `## Implementation` (Alternatives). Least-privilege-neutral: one read-only
`cat` of a specific user unit file, no new capability class, consistent with the siblings at
`vector.md:28-29`. Rollback-consistent: `git revert fe83073` restores the Q4 permission with the Q4
model lines. No permission expansion, no residue.
...
underlying gap (no commit in this repo is signature-verifiable) is real but pre-existing and out of
scope; the residual release-phase risk is the GPG nit below.
...
whose read-back is recorded 2026-09-09 in `## Implementation`. A reader checking the citation
finds a record that does not contain the current `ExecStart`, and the next model switch inherits
the same stale sentence. Full failure scenario (including the TASK-0010/TASK-0013 citation
evidence) in `## Archive`.
...
`add-ai-model` template provenance for the non-`ExecStart` lines.
**Resolution:** RESOLVED in `26b36f0` (parent `fe83073`), re-confirmed by `Shadow` (2026-09-09),
all three re-checks pass: sentence repointed (no 2026-08-29 date, no TASK-0010 citation, no
Q4-unit reference; the cited 2026-09-09 record contains the current `ExecStart`), both sweeps
re-ran clean with no stale reference reintroduced, and only `README.md` changed (3 insertions,
3 deletions). Per-check detail in `## Archive`.

### GPG text in plan item 10 and the `## Release` template is stale against the pre-approved unsigned deviation
**Severity:** nit
**Where:** plan item 10 (GPG-signed text) and the `## Release` template line
**Problem:** item 10 says "GPG-signed (`-S`, DoD requirement)" and the Release template reads
"Commits: GPG-signed", but the DoD box (Knuckles item) pre-approved the unsigned commit, and the
commit is in fact unsigned (`git show -s --format=%G? fe83073` → `N`).
**Failure scenario:** the underlying gap, the one real verification hole in this release (anyone
with push access to `metalllinux/team-chaotix` can produce a commit indistinguishable from the
team's), would never be tracked as a follow-up and would be lost in the handoff. Full text in
`## Archive`.
...
**Resolution:** handled as a release-phase item, not a code fix: `## Release` records the actual
signing state (unsigned, reason, per the DoD box) and tracks GPG key provisioning as follow-up 1.
The deviation itself is pre-approved and not reopened. (The original `Resolution:` template line
was never filled in by `Tails`; this closure supersedes it.)
...
**Resolution:** follow-up, not a blocker for this commit; tracked as `## Release` follow-up 3
(pin the HF revision in the documented download commands).

**Verified clean (no findings):**

- **`vector.md:27` ssh permission (focus 1): least-privilege-neutral, no new capability class,
  rollback-safe.** No-wildcard string allowing exactly one read-only `cat` of a named user unit
  file, run as `howard` without sudo; the unit carries no credentials; the reachable set shrank
  (q4 unit file out, iq4xs in); strictly weaker than the pre-existing siblings at `vector.md:28-29`;
  no injection surface (static literal). `git revert fe83073` restores the q4 rule and the q4 unit
  is `active`, so the restored permission stays functional. Concur with Shadow's judgment call.
  Full analysis in `## Archive`.
- **README additions carry no credential or secret (focus 2).** The SHA-256 values are published
  integrity anchors, not secrets. `192.168.1.106` is RFC1918 and already in the README before this
  commit (the `baseURL` line changed only the port). Port 8093 documents the same exposure class as
  the documented 8092 (both open per the 2026-09-09 live read). The provider JSON is verbatim from
  `~/.config/opencode/opencode.json:138-154` (config read directly): the entry has no `apiKey`
  field, consistent with llama.cpp's `/v1` requiring no credentials.
- **Secret scan.** `gitleaks` and `trufflehog` are not installed on this host, so the scan was a
  manual pattern grep (API keys, tokens, passwords, PEM blocks, PAT prefixes) over the committed
  files: no real secrets in any of the 12 files. `README.md:63` (`ghp_...`) and `README.md:170`
  (`<YOUR_TOKEN>`) are placeholders; the `ghp_fake_canary_token_...` at
  `.github/actions/gitleaks-action/run.sh:21` is the deliberate positive-control canary
...
- **Agent-file diffs are frontmatter-only.** One changed `model:` line for the nine non-vector
  agents, two for `vector.md` (line 4 + line 27); every permission block byte-identical to the
  parent commit except `vector.md:27`.
- **Residue sweep re-verified independently.** `git grep` of the DoD + extended patterns over
  `README.md AGENTS.md .opencode/` returns nothing; all remaining repo-wide hits sit in
  `planning/` (historical records, correctly retained).
- **The unauthenticated-inference-on-LAN exposure is pre-existing and already mitigated in the
  ...
  mitigation sentence still reads at `README.md:533-536`.
- **GPG-unsigned commit:** covered by Shadow's GPG nit (pre-approved deviation, pre-existing
...
|---|---|---|---|
| model grep (DoD 1) | the 10 agent frontmatter model lines | PASS | 10 matches, one per file, all on line 4, single unique value |
| frontmatter YAML parse (DoD 8, Plan 13a) | all 10 agent files parse as YAML with the model asserted | PASS | PyYAML 6.0.1, 10/10; `model` asserted, line-4 position and delimiters checked (script exits 1 on any failure mode) |
| stale-ref sweep, DoD verbatim (DoD 6) | no stale refs in tracked non-planning files | PASS | empty on worktree and re-run against the commit (`git grep … fe83073`) |
| stale-ref sweep, extended (Plan 10) | no `8092\|27b-q4\|3f227079\|\bQ4\b` in README/AGENTS/agents | PASS | empty on worktree and commit; negative control first (the same patterns hit across `planning/`, e.g. 17 in TASK-0010, 32 in TASK-0013), so the clean result is real, not a dead pattern |
| live endpoint (DoD 7, Plan 13b) | `curl http://192.168.1.106:8093/v1/models` | PASS | HTTP 200; lists `Qwen3.8-27B-UD-IQ4_XS` (only model on the endpoint; `n_ctx` 262144) |
| README provider JSON (DoD 4, Plan 13d) | the opencode client block parses and is correct | PASS | single key `evo-x2-qwen3.8-iq4xs`, `baseURL …:8093/v1`, model id present; deep-equal to the live config entry, so no invented values |
| AGENTS.md Model line (DoD 2) | line 17 wording | PASS | reads the DoD wording verbatim |
| README model statements (DoD 3) | `## Model` statement + EVO-X2 opener | PASS | read IQ4_XS / `evo-x2-qwen3.8-iq4xs` / 8093 / `--parallel 1` |
| README host facts (DoD 4, 5) | unit, model dir, SHAs, ExecStart, firewall | PASS | all match the `## Status` verified facts; ExecStart byte-identical; firewall subsection documents 8093/tcp with the 2026-09-09 verified state |
| canary (Validation 5, assumption A1) | the first post-commit dispatch runs on IQ4_XS | PASS | the Shadow and Omega passes ran post-commit; this `Big` dispatch reports the IQ4_XS model id, so changed frontmatter is picked up; no empty or 32k-truncated result in the window |
...
negative control for the sweeps. **No unit, integration, Docker, or Sparky matrix exists or applies
for this change**: a config+docs change with no build (12 markdown files, zero code), so there is
no test suite to run and none was requested; a truncated matrix is not being reported as full
coverage, there is simply no matrix. The firewalld-for-8093 item (Plan 13c) needed no action from
this pass: Tails resolved it with the exact verified rule (`## Implementation`), and the README
...
the endpoint, the provider JSON, and the canary were verified independently of Tails' records. The
DoD box for `Big` ("all harness checks PASS, with no silently dropped checks") is met. No code bugs
to send to `Tails`; no harness bugs of my own to carry. (At the time of this pass, Shadow's
should-fix on `README.md:486-488` was still open pending the item-14 fix pass; it is resolved in
`26b36f0`, re-confirmed in `## Review`.)
...
**House-style pass (AGENTS.md §10) over `README.md:391-629`. Clean, no fixes made.** Scans for
em/en dash, double hyphen in prose, banned words, colons introducing explanations, and
"Not only X, but also Y" all clean (the only banned-word hit repo-wide is the substring
false-positive "adjusted" at `README.md:330`, outside the changed sections). Scan detail in
`## Archive`.

**Accuracy pass, every changed line against the `## Status` verified facts. Clean.** The `## Model`
statement and EVO-X2 opener read the DoD wording; the model-files table (dir, SHA date 2026-09-09,
both SHAs re-matched character for character, `hf download` on the new gguf name, port paragraph on
8093 with the "only listener" clause removed), the systemd unit (`ExecStart` byte-identical to the
`## Status` verified line; provenance sentence repointed to the 2026-09-09 TASK-0022 read-back per
Shadow's should-fix, `26b36f0`), and the verification list all hold. The firewall live-state
paragraph was re-verified against an independent live read this pass: `ssh howard@192.168.1.106
'sudo firewall-cmd --list-all'` (2026-09-09) → `public` (default, active) on `eno1`, services
`cockpit dhcpv6-client ssh`, ports 8080-8088, 8090, 8092, 8093 (/tcp); every fact in the paragraph
holds, and the 8092/tcp omission is the documented Option B (not a completeness claim). The opencode
client JSON block is key-for-key identical to the live entry at
`~/.config/opencode/opencode.json:138-154` (config re-read directly this pass). Per-line detail in
`## Archive`.
...
**Checked and needed no change:** `AGENTS.md:17` (DoD wording verbatim, re-confirmed this pass),
all 10 agent frontmatters (identical `model:` line on line 4), the rest of `README.md` outside the
two changed sections (no issue introduced by this task; pre-existing style debt elsewhere, e.g.
colons before lists at `README.md:38`, `:87`, `:98`, left alone as out of scope), and worktree vs
commit (`git diff --stat 26b36f0 -- README.md AGENTS.md .opencode/agents` → empty, byte-identical
to the feature-branch tip).
...
|---|---|---|
| 2026-09-09 | `## Implementation`: "Checks run" compressed from per-command session log to command + result (30 → 20 lines); "Fix pass" compressed (13 → 7 lines). Detail below. | ~43 lines |
| 2026-09-09 | `## Review`: per-bullet re-verification (46 → 23 lines, the two judgment calls kept as decisions); should-fix failure scenario + 3 re-checks (39 → 18 lines); GPG nit failure scenario (18 → 17 lines, unfilled `Resolution:` template replaced by the closure per `## Release` follow-up 1). Detail below. | ~80 lines |
| 2026-09-09 | `## Security`: verified-clean analysis compressed (60 → 48 lines; license-compliance notes and the low HF-ref finding kept intact, its unfilled `Resolution:` line replaced by the `## Release` follow-up-3 pointer). `vector.md:27` full analysis below. | ~12 lines |
| 2026-09-09 | `## Test Results`: table Notes compressed (raw output → command + outcome); "Checks requested vs run" and Verdict tightened; the stale "should-fix unresolved" note annotated as resolved in `26b36f0`. | ~10 lines |
| 2026-09-09 | `## Docs`: house-style scan results and accuracy-pass per-line detail (25 → 16 lines, the "other quantizations" judgment call kept); "Checked and needed no change" list compressed (9 → 6 lines). Scan/accuracy detail below. | ~22 lines |
| 2026-09-09 | `## Plan`: work-breakdown table + critical path + estimates (52 → 8 lines); assumptions A1-A7 (15 → 7 lines, all resolved); risks 1-7 + validation criteria (57 → 15 lines, risk 4 and risk 2 outcomes kept). Original text below. | ~110 lines |
| 2026-09-09 | `## Next Actions`: the done Espio pruning item replaced by a one-line closure note. | 3 lines |

### Implementation — "Checks run" full session detail (compressed 2026-09-09)
...
### Implementation — "Fix pass" full text (compressed 2026-09-09)
...
### Review — per-bullet re-verification detail (compressed 2026-09-09)
...
### Review — should-fix failure scenario and re-check detail (compressed 2026-09-09)
...
(`planning/docs/TASK-0010-evox2-gpu-wedge-fix.md:505,513`), and the 2026-08-29 read-back the
...
Re-checks against `26b36f0` (all pass):
...
### Review — GPG nit full failure scenario (compressed 2026-09-09)

Knuckles fills `## Release` from the template and either leaves "GPG-signed" as-is, so the release
record contradicts both the actual commit and the DoD box, or ticks the DoD box while item 10's
stale text reads as an unmet requirement. Either way the underlying gap, the one real
verification hole in this release (anyone with push access to `metalllinux/team-chaotix` can
produce a commit indistinguishable from the team's), is never tracked as a follow-up task and is
lost in the handoff.

### Security — `vector.md:27` full analysis (compressed 2026-09-09)

The permission is a no-wildcard string, so it allows exactly one command. That command is a
read-only `cat` of one named user unit file, run as `howard` without sudo
...
### Docs — house-style scan results and accuracy-pass per-line detail (compressed 2026-09-09)

House-style scans over the README:
...
### Plan — work breakdown table, critical path, estimates (archived 2026-09-09)

One agent finishes one item in one turn. The single inference slot serializes everything; "parallel with" means freely orderable.
...
### Plan — assumptions A1-A7 original text (archived 2026-09-09)
...
### Plan — risks 1-7 and validation criteria original text (archived 2026-09-09)
